...
onready var _animator = _body.get_node("CharacterAnimator")
...
func _move(delta : float):
	...
	_animator.animate_movement(input)
		
	input = input.normalized()

	var direction = input.rotated(Vector3.UP, deg2rad(_body.rotation_degrees.y))
	...
	_body.move_and_slide(_velocity, Vector3.UP)
...
func _turn(delta : float):
	if not Input.is_mouse_button_pressed(BUTTON_RIGHT):
		...
		if Input.is_action_pressed("turn_left"):
			...
			dir = -1
		_body.rotate(Vector3.UP, dir * key_turn_speed * delta)
	else:
		_body.rotate(Vector3.UP, -clamp(_mouse_delta_x, -mouse_turn_speed, mouse_turn_speed) * delta)
		_mouse_delta_x = 0
